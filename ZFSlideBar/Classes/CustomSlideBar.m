...
- (void)viewDidLoad {
    ...
    //列表
    CGRect rect = CGRectMake(kScreenWidth, 0, kSidebarWidth, self.view.bounds.size.height);
...
- (void)showHideSidebar{
    if (self.contentView.frame.origin.x == kScreenWidth) {
...
//        NSLog(@"自动弹出");
        ...
        [UIView animateWithDuration:duration animations:^{
            [self setSidebarOriginX:kScreenWidth - kSidebarWidth];
...
//        NSLog(@"自动缩回");
        [UIView animateWithDuration:duration animations:^{
            [self setSidebarOriginX:kScreenWidth];
        } completion:^(BOOL finished) {
            _isMoving = NO;
            self.view.hidden = YES;
            [self slideToRight];
